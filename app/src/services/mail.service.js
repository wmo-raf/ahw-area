...
class MailService {
    constructor(publicUrl, disableEmailSending = false) {
        ...
        if (API_KEY) {
            ...
            logger.info(
                "Skipping init of Sparkpost client due to missing API key."
            );
            this.disableEmailSending = true;
        }
    }

    async sendMail(template, data, recipients, sender = "ahw") {
        logger.info(
            `[MailService] Sending areas "${template}" email to `,
            recipients
        );
        const reqOpts = {
        ...
        if (this.disableEmailSending) {
            logger.info(
                `[MailService] Email sending disabled, skipping ${template} email`
            );
            logger.info(reqOpts);
