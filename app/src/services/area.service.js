...
class AreaService {
    static async getAll(query = {}, user) {
        ...
        const ids = query.ids
            ? query.ids.split(",").map((el) => el.trim())
            : [];
        logger.debug(`ids param: ${ids}`);
        const filteredQuery = AreaService.getFilteredQuery(
    ...
    static async getByDataset(resource) {
        logger.debug(
            `[AreaService] Getting areas for datasets with ids ${resource.ids}`
        );
        if (resource.app) {
    ...
    static async hasPermission(id, user) {
        ...
        const appPermission = area.application.find((areaApp) =>
            user.extraUserData.apps.find((app) => app === areaApp)
        );
        if (!appPermission) {
            permission = false;
        }
        if (
            ...
            (!area.userId || area.userId !== user.id)
        ) {
            permission = false;
        }
        return permission;
